...
class DetailSoalFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Ambil list soal dan waktu dari Bundle
        ...
        val btnLanjut = view.findViewById<Button>(R.id.btn_Berikutnya)
    ...
    private fun tampilkanSoal(view: View) {
        ...
        // Update tombol kembali dan kirim
        ...
        // Mengecek ukuran gambar dan menampilkannya menggunakan Glide
        val imgSoal = view.findViewById<ImageView>(R.id.imgSoal)
        val imageUrl = soal.gambar
        if (!imageUrl.isNullOrEmpty()) {
            if (isValidImageSize(imageUrl)) {
                imgSoal.visibility = View.VISIBLE
                Glide.with(this)
                    .load(imageUrl)
                    .error(R.drawable.placeholder_image) // tampilkan placeholder kalau gagal load
                    .into(imgSoal)
            } else {
                imgSoal.visibility = View.GONE // gambar terlalu besar, sembunyikan aja
            }
        } else {
            imgSoal.visibility = View.GONE // guru tidak upload gambar, jadi disembunyikan
        }
    }
    ...
    private fun kirimJawaban() {
        ...
        requireActivity().supportFragmentManager.beginTransaction()
            .replace(R.id.frame_container, fragment) // Ganti dengan ID container Anda
            .addToBackStack(null) // Agar fragment baru dapat kembali dengan tombol back
            .commit()
    }
    ...
    private fun hitungSkor(): Int {
        ...
        listSoal.forEachIndexed { index, soal ->
            if (jawabanUser[index] == soal.jawabanBenar) skor += 10
        }
        return skor
    }

    private fun getImageFileSize(url: String): Long {
    ...
    private fun isValidImageSize(url: String): Boolean {
        val fileSize = getImageFileSize(url)
        return fileSize <= 1_048_576 // 1 MB = 1,048,576 bytes
    }
